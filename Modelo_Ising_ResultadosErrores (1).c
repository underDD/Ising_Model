    ...
    if (fout==NULL) printf("ERROR");
        ...
        for(sentido=0;sentido<1;sentido++){ //Recorrerlas betas hacia delante y hacia atras.
            ...
            for(Nbeta=0;Nbeta<N_pasos;Nbeta++){ //Caulculos para cada beta
                ...
                for(N_term=0;N_term<N_t;N_term++){ //Tras la termalizaci�n se supone que obtenemos configuraciones mas centradas en la media pero hay que hallar el valor de N_t
...
float error_bloques(float *e, int N_med, int P){
    int N = N_med / P;//P (el n�mero de bloques) tiene que ser divisor de N_med, N es el n�mero de datos por bloque
    ...
    for(int p=0;p<P;p++){
        ...
        med_var(prov,N,&media_bloques[p],&var);//ahora vale para calcular e y m, habr�a que cambiar esta l�nea si se quiere con Cv y ji, pero no hay subalgoritmos que lo hagan :(
    }
